class Matrix {
  ...
  Matrix relu(){
    float[][] relu = new float[rows][cols];//this is the matrix that will store the relu activated nodes
    for(int r = 0; r < rows; r++){
      for(int c = 0; c < cols; c++){//lol, c++
        if(mat[r][c] < 0){
          relu[r][c] = 0;
  ...
  Matrix mutate(float mutationRate, float learningRate){
    float[][] mutated = copy();
    
    for(int r = 0; r < rows; r++){
      for(int c = 0; c < cols; c++){
        if(Math.random() < mutationRate){
          mutated[r][c] += (Math.random()*2-1) * learningRate;
        }
      }
    }
    return new Matrix(mutated);
  }
